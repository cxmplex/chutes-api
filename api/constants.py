...
class ServerHealthStatus(str, Enum):
    """TEE endpoint health derived from the server's last successful conforming probe."""

    HEALTHY = "healthy"  # last successful probe within the degraded threshold
    DEGRADED = "degraded"  # no successful probe past the degraded threshold (e.g. >12h)
    OFFLINE = "offline"  # no successful probe past the offline threshold (e.g. >72h)
    UNKNOWN = "unknown"  # last_health_at IS NULL — no successful probe recorded
