...
class ReissueReleaseTargetTokensRequest(BaseModel):
    """Explicitly rotate one or more logical-target token generations for a launcher."""

    host_id: str = Field(..., min_length=1, description="Enrolled logical L0 launcher label.")
    tee_type: str = Field(..., description="The launcher's configured TEE type.")
    channel: str = Field(RELEASE_CHANNEL_DEFAULT, min_length=1)
    roles: List[str] = Field(
        ...,
        min_length=1,
        description=(
            "Logical roles whose current token generation must be invalidated and replaced. "
            "This is same-miner operational telemetry and does not establish physical placement."
        ),
    )
    ...
    @classmethod
    def _valid_roles(cls, values: List[str]) -> List[str]:
        normalized = [(value or "").strip().lower() for value in values]
        if any(value not in {"chute", "storage"} for value in normalized):
            raise ValueError("roles may contain only chute and storage")
...
class ReleaseManifest(BaseModel):
    ...
    target_tokens: Dict[str, str] = Field(
        default_factory=dict,
        description=(
            "Current validator-signed one-use token generations for this enrolled logical target, "
            "keyed by release role. They are same-miner-transferable and do not prove physical-host "
            "identity."
        ),
    )
...
class LogicalReleaseTargetStatus(BaseModel):
    ...
    same_miner_token_transferable: bool = Field(
        True,
        description=(
            "Always true for Model B: another unattested L0 controlled by the same miner can carry "
            "this logical target token and claim the target label."
        ),
    )
...
class ReleaseStatusResponse(BaseModel):
    ...
    logical_target_tokens_same_miner_transferable: bool = Field(
        True,
        description=(
            "Always true: the unattested L0 makes logical target bearer tokens transferable between "
            "physical machines controlled by the same miner."
        ),
    )
