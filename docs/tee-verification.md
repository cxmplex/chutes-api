...
**Certificate** -- The TLS certificate (base64-encoded DER) of the server hosting the instance. Its public key is TDX-attested (the quote binds to it) and is the key whose corresponding private key signed the `signature` field, closing the key-possession proof.

**Signature** -- A base64-encoded RSA-PKCS1v15-SHA256 signature of `attested_body`, signed by the host TLS private key. Present on attestation proxy >= 0.2.0. Verifying this against the certificate's public key proves the entity that returned the response is in possession of the private key attested to in the TDX quote — a direct proof of responder identity.

**Attested Body** -- The base64-encoded raw response bytes returned by the attestation proxy — the exact byte sequence that `signature` covers. Required for signature verification since the Chutes API re-serializes the data before returning it to clients. Present when `signature` is present.
  ...
  "evidence": [
    {
      ...
      "attested_body": "<base64-encoded raw attestation proxy response bytes>"
    }
  ],
...
| `evidence[].signature`       | string | *(Optional)* Base64-encoded RSA-PKCS1v15-SHA256 signature of `attested_body`, signed with the host TLS private key. Present on attestation proxy >= 0.2.0. |
| `evidence[].attested_body`   | string | *(Optional)* Base64-encoded raw response body from the attestation proxy — the exact bytes covered by `signature`. Present when `signature` is present. |
...
| `signature`      | string | *(Optional)* Base64-encoded RSA-PKCS1v15-SHA256 signature of `attested_body`, signed with the host TLS private key. Present on attestation proxy >= 0.2.0. |
| `attested_body`  | string | *(Optional)* Base64-encoded raw response body from the attestation proxy — the exact bytes covered by `signature`. Present when `signature` is present.     |
...
### Step 6: Verify Key Possession (X-Signature)

When `signature` and `attested_body` are present (attestation proxy >= 0.2.0), you can verify that the entity returning the attestation evidence holds the TLS private key corresponding to the certificate that is bound to the TDX quote. This closes the identity loop: the quote binds to the certificate, and the signature proves possession of the private key.
...
instance_evidence = data["evidence"][0]  # or data for single-instance endpoint

signature = instance_evidence.get("signature")
attested_body = instance_evidence.get("attested_body")

if signature and attested_body:
    cert = x509.load_der_x509_certificate(base64.b64decode(instance_evidence["certificate"]))
    pub_key = cert.public_key()
    sig_bytes = base64.b64decode(signature)
    body_bytes = base64.b64decode(attested_body)
    try:
        pub_key.verify(sig_bytes, body_bytes, padding.PKCS1v15(), hashes.SHA256())
        print("Key possession verified: response was signed by the attested TLS private key")
    except InvalidSignature:
        raise ValueError("Signature verification failed: response may not have come from the attested instance")
else:
    print("Signature fields absent (attestation proxy < 0.2.0); key possession check skipped")
```

This step proves that whoever sent the response is in possession of the private key that was attested to in the TDX quote — providing a direct cryptographic link between the hardware attestation and the HTTP response.
...
def verify_signature(instance_evidence: dict) -> bool:
    """Verify the X-Signature key-possession proof (attestation proxy >= 0.2.0).

    Returns True if verified, False if signature fields are absent (older proxy).
    Raises ValueError on a verification failure.
    """
    signature = instance_evidence.get("signature")
    attested_body = instance_evidence.get("attested_body")
    if not signature or not attested_body:
        return False
    ...
    try:
        ...
        raise ValueError("X-Signature verification failed: response was not signed by the attested TLS private key")
    return True
...
def main():
    ...
    for instance_evidence in chute_evidence["evidence"]:
        ...
        # Verify key possession via X-Signature (proxy >= 0.2.0)
        try:
            sig_verified = verify_signature(instance_evidence)
            if sig_verified:
                print(f"  Key possession (X-Signature): PASSED")
            else:
                print(f"  Key possession (X-Signature): SKIPPED (attestation proxy < 0.2.0)")
        except ValueError as e:
            print(f"  Key possession (X-Signature): FAILED ({e})")
            all_passed = False
...
| RSA signature over `attested_body`            | The entity that returned the response holds the TLS private key corresponding to the TDX-attested certificate (key-possession proof). Present on attestation proxy >= 0.2.0. |
...
| `cryptography`       | X.509 certificate parsing and RSA signature verification for the key-possession proof (Step 6).                   |
